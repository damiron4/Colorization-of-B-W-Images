...
def split_set(root, out_root, a_size, b_size=None, a_name='train', b_name='test', chosen_categories_src=None):
    chosen_classes = [line.rstrip('\n') for line in open(chosen_categories_src, 'r')]

    print(f"Splitting into {out_root}{a_name}")
    if b_size:
        print(f"Splitting into {out_root}{b_name}")
    ...
    for c in classes_dirs:
        if c in chosen_classes:
            ...
            # Ensure the parent directory exists before creating the leaf directory
            os.makedirs(os.path.dirname(classes_path_out_train), exist_ok=True)
            ...
            if b_size:
                ...
                # Ensure the parent directory exists before creating the leaf directory
                os.makedirs(os.path.dirname(classes_path_out_test), exist_ok=True)
