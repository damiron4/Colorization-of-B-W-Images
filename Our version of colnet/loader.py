...
from src.our_colnet import ColNet #our model
...
def load_config(config_file, model_checkpoint=None):
    ...
    with open(config_file, 'r') as conf:
        print(conf)
        y = yaml.load(conf, Loader=yaml.FullLoader)
...
if __name__ == "__main__":
    ...
    save_path = './plot_dl/tsne/places12/' #path to save t-SNE
    print(args.config)
    t = load_config(args.config, args.model)
    t.info()
    t.run()
    t.test()
    t.visualize_tsne(save_path=save_path) #Added 
