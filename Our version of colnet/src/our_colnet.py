...
class LowLevelFeatures(nn.Module):
    ...
    def __init__(self, net_divisor=1):
        ...
        self.dropout = nn.Dropout(p=0.5)            # Our own dropout layer
...
class MidLevelFeatures(nn.Module):
    ...
    def forward(self, x):
        out = F.PReLU(self.conv7(x))
        out = F.PReLU(self.conv8(out))
        return out
...
class GlobalFeatures(nn.Module):
    ...
    def __init__(self, net_divisor=1):
        ...
        self.fc3 = nn.Linear(ksize[0], ksize[3])
        


    def forward(self, x):
        y = F.PReLU(self.conv1(x))
        y = F.PReLU(self.conv2(y))
        y = F.PReLU(self.conv3(y))
        y = F.PReLU(self.conv4(y))
        ...
        # Branching
        y = y.view(-1, self.ksize0)

        out = y
        classification_in = y
            
        out = F.PReLU(self.fc3(out))
...
class ColorizationNetwork(nn.Module):
    ...
    def forward(self, x):
        out = F.PReLU(self.conv9(x))

        # Upsample #1        
        out = nn.functional.interpolate(input=out, scale_factor=2)

        out = F.PReLU(self.conv10(out))
        out = F.PReLU(self.conv11(out))
        
        # Upsample #2
        out = nn.functional.interpolate(input=out, scale_factor=2)

        out = F.PReLU(self.conv12(out))
        out = torch.sigmoid(self.conv13(out))
        
        # Upsample #3
        out = nn.functional.interpolate(input=out, scale_factor=2)
        
        #Tried to do Grad-CAM but did not have enough time
#         Generate Grad-CAM
...
class ColNet(nn.Module):
    ...
    def fusion_layer(self, mid_out, glob_out):
        ...
        out = F.PReLU(self.conv_fuse(stack_volume))
        return out
